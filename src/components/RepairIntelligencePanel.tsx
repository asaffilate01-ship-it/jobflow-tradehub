  ...
  return (
    <section className={`rounded-2xl border p-5 sm:p-6 ${diagnosis?.emergency_stop ? "border-destructive/30 bg-destructive/5" : "border-primary/20 bg-primary/5"}`}>
      ...
      {diagnosis ? <div className="mt-5 grid gap-4 md:grid-cols-3">
        <div><div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground"><Wrench className="h-3.5 w-3.5" />Likely causes</div><ul className="mt-2 space-y-1 text-sm">{(diagnosis.probable_causes ?? []).map((cause: string) => <li key={cause}>â€¢ {cause}</li>)}</ul></div>
        <div><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">Indicative cost</div><div className="mt-2 text-2xl font-bold">Â£{cost.minimum ?? "?"}â€“Â£{cost.maximum ?? "?"}</div><div className="mt-1 text-xs text-muted-foreground capitalize">Suggested: {diagnosis.suggested_trade?.replace("_", " ")}</div></div>
        <div><div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5" />Safety</div>{diagnosis.emergency_stop ? <ul className="mt-2 space-y-1 text-sm text-destructive">{(diagnosis.safety_actions ?? []).map((action: string) => <li key={action}>â€¢ {action}</li>)}</ul> : <p className="mt-2 text-sm text-muted-foreground">No emergency stop was triggered. Stay away from anything unsafe and wait for the professional assessment.</p>}</div>
      </div> : <p className="mt-4 text-sm text-muted-foreground">Assessment is being prepared.</p>}

      {diagnosis?.likely_remedies?.length > 0 && <div className="mt-4 border-t border-border/60 pt-4"><div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">How it may be repaired</div><ul className="mt-2 space-y-1 text-sm">{diagnosis.likely_remedies.map((remedy: string) => <li key={remedy}>â€¢ {remedy}</li>)}</ul></div>}
