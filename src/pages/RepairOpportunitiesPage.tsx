          ...
          <section key={invite.id} className="glass-card overflow-hidden">
            <div className="space-y-4 p-5 sm:p-6">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                {payload.estimated_cost && <div className="text-right"><div className="text-xs text-muted-foreground">AI indication</div><div className="font-bold">£{payload.estimated_cost.minimum ?? "?"}–£{payload.estimated_cost.maximum ?? "?"}</div></div>}
              </div>
            ...
            {canRespond && activeInvite === invite.id && (
              <div className="border-t bg-secondary/20 p-5 sm:p-6">
                ...
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <label className="space-y-1 text-xs font-medium">Labour (£)<Input type="number" min="0" value={offer.labour} onChange={(event) => setOffer({ ...offer, labour: event.target.value })} /></label>
                  <label className="space-y-1 text-xs font-medium">Materials (£)<Input type="number" min="0" value={offer.materials} onChange={(event) => setOffer({ ...offer, materials: event.target.value })} /></label>
