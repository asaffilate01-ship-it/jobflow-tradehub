...
-- Safety-first intake, four-provider private dispatch, structured offers and evidence sync.
...
VALUES ('repair-intake', 'repair-intake', false, 52428800, ARRAY['image/jpeg','image/png','image/webp','video/mp4','video/webm','audio/mpeg','audio/webm'])
...
BEGIN
  ...
  UPDATE public.repair_dispatch_invites SET status = CASE WHEN trade_company_id = v_quote.trade_company_id THEN 'accepted' ELSE 'not_selected' END WHERE dispatch_round_id = v_round_id;
  INSERT INTO public.repair_integration_outbox (event_type, aggregate_type, aggregate_id, destination, payload, idempotency_key)
  VALUES ('offer.accepted', 'job', v_job.id, 'dokuvera', jsonb_build_object('job_id',v_job.id,'quote_id',p_quote_id,'trade_company_id',v_quote.trade_company_id), 'offer.accepted:'||v_job.id::text||':dokuvera')
  ON CONFLICT (idempotency_key) DO NOTHING;
  INSERT INTO public.repair_integration_outbox (event_type, aggregate_type, aggregate_id, destination, payload, idempotency_key)
  SELECT 'offer.accepted', 'job', v_job.id, v_job.source_product,
    jsonb_build_object('job_id',v_job.id,'quote_id',p_quote_id,'trade_company_id',v_quote.trade_company_id,'source_reference',v_job.source_reference),
    'offer.accepted:'||v_job.id::text||':'||v_job.source_product
  WHERE v_job.source_product IN ('gabley','immoviq')
  ON CONFLICT (idempotency_key) DO NOTHING;
  RETURN v_job.id;
END;
$$;
...
-- Providers can maintain coverage/availability, but only administrators or the
-- service role can assert verification. Editing credentials invalidates the
-- corresponding previous verification.
